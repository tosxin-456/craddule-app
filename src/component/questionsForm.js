...
function QuestionsForm() {
    ...
    const shouldShowWordCounter = (question, questionsArray) => {
        ...
        const questionIndex = questionsArray.findIndex(q => q._id === question._id);
    ...
    const checkWordCountRequirements = () => {
        ...
        for (let i = 0; i < questions.length; i++) {
            ...
            if (!isQuestion1 && isLastTwoQuestions) { // Skip validation for question 1, but check last 2
    ...
    const getInsufficientWordCountQuestions = () => {
        ...
        for (let i = 0; i < questions.length; i++) {
            const question = questions[i];
            const isQuestion2Or5 = question.questionOrder === "2" || question.questionOrder === "5";
    ...
    function getWordlistSuggestion(word) {
        ...
        for (let i = 0; i < allWords.length; i++) {
            ...
            if (dist < min && dist <= 2 && dist > 0) { // dist > 0 ensures we don't suggest exact matches
                best = candidate;
                min = dist;
            }
            if (min <= 0.5) break;
    ...
    // Modified handleTextChange - removed automatic wordlist corrections
    const handleTextChange = (e, questionId) => {
        ...
        if (endsWithSpace && lastWord) {
            ...
            // Step 2: Smart prediction (removed fuzzy correction)
    ...
    const getSuggestionText = (questionId) => {
        ...
        const words = answers[questionId].trim().split(/\s+/).filter(w => w.length > 0);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!checkWordCountRequirements()) {
            ...
            setErrorMessage(`Please ensure the last 2 questions have at least 100 words. Question(s) ${insufficientQuestions.join(', ')} need more content.`);
            return;
        }
            ...
            <div className="bg-white w-full max-w-4xl rounded-xl shadow-xl p-4 sm:p-6 md:p-8">
                <div className="text-center mb-6 sm:mb-8">
                    ...
                    <p className="text-gray-600 text-base sm:text-lg leading-relaxed max-w-3xl mx-auto mb-4 sm:mb-6 px-2 sm:px-0">
                        To help us personalise your environment, we need to understand certain aspects of this idea you have. Please fill in the following questions - the more robustly you answer them, the better the output will be.
                    </p>
                </div>
                ...
                {loading ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
                                ...
                                <div key={question._id} className="space-y-3 sm:space-y-4">
                                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-0">
                                        ...
                                        {showWordCounter && (
                                            <div className="flex-shrink-0 sm:ml-4 w-full sm:w-auto">
                                                <div className={`border rounded-lg p-2 sm:p-3 sm:min-w-[220px] ${wordCount >= 100
                                                    ? 'bg-green-50 border-green-200'
                                                    : 'bg-yellow-50 border-yellow-200'
                                                    }`}>
                                                    <div className={`text-xs sm:text-sm font-medium mb-2 text-center ${wordCount >= 100
                                                        ? 'text-green-700'
                                                        : 'text-yellow-700'
                                                        }`}>
                                                    ...
                                                    <div className="w-full bg-gray-200 rounded-full h-2 sm:h-2.5 mb-2">
                                                        <div
                                                            className={`h-2 sm:h-2.5 rounded-full transition-all duration-300 ease-out ${wordCount >= 100
                                                                ? 'bg-green-500'
                                                                : 'bg-yellow-500'
                                                                }`}
                                                    ...
                                                    {/* Progress percentage */}
                                                    <div className={`text-xs font-medium text-center mb-2 ${wordCount >= 100 ? 'text-green-600' : 'text-yellow-600'
                                                        }`}>
                                                        {Math.round(progress)}% complete
                                                    </div>

                                                    {encouragingMessage && (
                                                        <div className={`text-xs sm:text-sm font-medium px-2 sm:px-3 py-1 sm:py-2 rounded-md ${wordCount < 100
                                    ...
                                    <div className="relative">
                                        ...
                                        <textarea
                                            ...
                                            className={`relative w-full p-3 sm:p-4 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 resize-vertical min-h-[100px] sm:min-h-[120px] bg-white text-sm sm:text-base ${showWordCounter && wordCount < 100
                                                ? 'border-yellow-300'
                                                : 'border-gray-300'
                                                }`}
                                            placeholder={`Enter your answer here... ${showWordCounter ? '(Minimum 100 words required)' : ''} (Smart corrections and suggestions enabled)`}
                                            value={currentAnswer}
                        ...
                        <div className="pt-4 sm:pt-6 border-t border-gray-200">
                            <button
                                type="submit"
                                className={`w-full rounded-lg py-3 sm:py-4 px-4 sm:px-6 font-semibold text-base sm:text-lg flex items-center justify-center transition-all duration-200 transform ${isSubmitDisabled
                            ...
                            {!checkWordCountRequirements() && (
                                <p className="text-red-600 text-xs sm:text-sm text-center mt-2 sm:mt-3 px-2 sm:px-0">
                                    Please ensure questions 2 and 5 have at least 100 words before submitting.
                                </p>
                            )}
                        </div>
                    </form>
