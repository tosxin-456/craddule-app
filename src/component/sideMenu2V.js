...
const SideMenu2 = () => {
  ...
  const [percentageDE, setPercentageDE] = useState(null);
    ...
    const onClickStatistics = () => navigate(`/validate/start`);
  const toggleMenu = () => {
      ...
      <div className={`side-menu ${!isCollapsed ? "collapsed" : ""}`}>
       ...
       <li onClick={onClickStatistics}>
          ...
          <li onClick={toggleDropdownF}>
            ...
            {isCollapsed && (
              <span>
                Full Product/Project Review{" "}
                {percentage !== null && ` (${percentage}%)`}
              </span>
            )}
          </li>
          {isDropdownOpenF && !isCollapsedF && (
            <ul className="dropdown">
              ...
              <li
                className="dropDownNew"
                onClick={() => onClickCHPd("FullProductProjectReview")}
              >
                Summary PDF
              </li>
            </ul>
            ...
            <ul className="dropdown">
              {subTypesE.map((subType, index) => (
                <li
                  key={index}
                  className="dropDownNew"
                  onClick={() => handleSubTypeClickE(subType.subCategory)}
                >
                  {subType.subCategoryName}
                </li>
              ))}
              <li
                className="dropDownNew"
                onClick={() => onClickCHPd("DetailedMarketingRtmTesting")}
              >
                Summary PDF
              </li>
            </ul>
          ...
          <li onClick={toggleDropdownB}>
            ...
            {isCollapsed && (
              <span>
                ...
                {percentageDE !== null && ` (${percentageDE}%)`}
              </span>
            )}
          </li>
            ...
            <ul className="dropdown">
              {subTypesB.map((subType, index) => (
                <li
                  key={index}
                  className="dropDownNew"
                  onClick={() => handleSubTypeClickB(subType.subCategory)}
                >
                  {subType.subCategoryName}
                </li>
              ))}
              <li
                className="dropDownNew"
                onClick={() => onClickCHPd("DevelopmentCostReview")}
              >
                Summary PDF
              </li>
            </ul>
