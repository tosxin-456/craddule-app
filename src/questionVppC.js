...
function QuestionBus() {
    ...
    const questionSubType ="Conclusion";
    ...
    const fetchUnansweredQuestion = async () => {
        try {
          const response = await fetch(API_BASE_URL+`/api/new/question/${userId}/${projectId}/ValuePropositionPack/Conclusion`);
          if (response.status === 200) {
            const data = await response.json();
            if (!data.data) {
            //   setNoMoreQuestions(true);
                navigate(`/questionVppCSum`);
            } else {
    ...
    <div className='container2'>
         ...
         <div className='text-center'>
                    <p className='textHp'>Conclusion</p>
