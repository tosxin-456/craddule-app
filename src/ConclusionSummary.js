...
const ConclusionSummaryViewer = () => {
    ...
    const formatMarkdown = (text) => {
        ...
        html = html.replace(/\|(.+)\|\n\|[\s\-\|]+\|\n((?:\|.+\|\n?)*)/g, (match, header, rows) => {
            ...
            // Body - smaller padding on mobile
            tableHtml += '<tbody>';
        ...
        // Convert only H4 and H3 headers - removed H1 and H2 conversions
        html = html.replace(/^#{4}\s+(.*$)/gm, '<h4 class="text-base sm:text-lg font-semibold text-gray-800 mt-6 sm:mt-8 mb-3 sm:mb-4 pb-2 border-b border-gray-200">$1</h4>');
        html = html.replace(/^#{3}\s+(.*$)/gm, '<h3 class="text-lg sm:text-xl font-bold text-gray-900 mt-8 sm:mt-10 mb-4 sm:mb-6 pb-3 border-b-2 border-blue-200">$1</h3>');
        ...
        // Convert bullet points with better mobile spacing
        html = html.replace(/^\s*[\*\-]\s+(.*)$/gm, '<li class="ml-4 sm:ml-6 mb-2 text-gray-700 text-sm sm:text-base relative"><span class="absolute -left-3 sm:-left-4 text-blue-500">•</span>$1</li>');

        // Convert numbered lists with better mobile spacing
        html = html.replace(/^(\d+)\.\s+(.*)$/gm, '<li class="ml-5 sm:ml-6 mb-2 text-gray-700 text-sm sm:text-base relative"><span class="absolute -left-5 sm:-left-6 text-blue-600 font-medium">$1.</span>$2</li>');

        // Wrap lists
        html = html.replace(/(<li class="ml-[45] sm:ml-6 mb-2[^>]*>.*<\/li>\s*)+/g, '<ul class="space-y-1 my-3 sm:my-4">$&</ul>');

        // Convert paragraphs with mobile-responsive text
        html = html.replace(/\n\n/g, '</p><p class="mb-3 sm:mb-4 text-gray-700 text-sm sm:text-base leading-relaxed">');

        // Wrap in initial paragraph if it doesn't start with a tag
        if (!html.startsWith('<')) {
            html = '<p class="mb-3 sm:mb-4 text-gray-700 text-sm sm:text-base leading-relaxed">' + html;
        }
        if (!html.endsWith('>')) {
            html = html + '</p>';
        }
            ...
            <div className="mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-8 max-w-full">
                ...
                {!summary && !loading && !error && (
                    <div className="bg-white rounded-lg shadow-sm p-6 sm:p-8 md:p-12 text-center mx-2 sm:mx-0">
                        ...
                        <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">No Summary Available</h3>
                        <p className="text-gray-600 mb-6 text-sm sm:text-base px-2 sm:px-4 leading-relaxed">
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
                            <div className="flex items-center justify-between">
                                <div className="min-w-0 flex-1 pr-3">
                                    <p className="text-xs sm:text-sm font-medium text-gray-600 mb-1">Document Type</p>
                                    <p className="text-sm sm:text-lg font-semibold text-gray-900 truncate">Investor Report</p>
                                </div>
