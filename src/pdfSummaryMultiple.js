...
function QuestionBusIntro() {
 ...
 const [showImagePopup, setShowImagePopup] = useState(false);
    ...
    const [answersVVP, setAnswersVPP] = useState([]);
    const [answersSM, setAnswersSM] = useState([]);
    const [answersDMS, setAnswersDMS] = useState([]);
    const [hoveredIndex, setHoveredIndex] = useState(null);
 const [loading, setLoading] = useState(false);
 const [error, setError] = useState(null);
 const projectId = localStorage.getItem('nProject');
 const [combinedAnswer, setCombinedAnswer] = useState('');
 const [combinedAnswerVPP, setCombinedAnswerVPP] = useState('');
 const [combinedAnswerSM, setCombinedAnswerSM] = useState('');
 const [combinedAnswerDMS, setCombinedAnswerDMS] = useState('');
...
useEffect(() => {
  ...
  const fetchAnswers = async () => {
    const category = 'BusinessAnalysisPack';
    try {
 ...
 useEffect(() => {
  console.log("fetchAnswers");
  const fetchAnswers = async () => {
    const category = 'ValuePropositionPack';
    try {
      const summaryResponse = await fetch(API_BASE_URL + `/api/pdf/${projectId}/${category}`, {
        headers: {
          'Content-Type': 'application/json',
      ...
      if (summaryResponse.ok) {
        ...
        if (dataS.data) {
          console.log(dataS);
          console.log(dataS.data.summary);
          setCombinedAnswerVPP(dataS.data.summary);
        } else {
          const response = await fetch(API_BASE_URL + `/api/pdf/single/${projectId}/${category}`, {
            headers: {
              'Content-Type': 'application/json',
          ...
          console.log(data.data[0].summary);
          console.log('answers getting because no summary');
          setAnswersVPP(data.data);
          const combined = data.data.map(answer => {
            const formattedSubType = answer.questionSubType.replace(/([A-Z])/g, ' $1').trim();
            return `<h4 style="text-align: center;">${formattedSubType}</h4><br>${answer.summary}`;
          }).join('<br><br>');
          console.log(combined);
          setCombinedAnswerVPP(combined);
          setLoading(false);
        }
      } else {
        throw new Error('Failed to fetch summary');
    ...
    try {
      ...
      if (summaryResponse.ok) {
        ...
        if (dataS.data) {
          console.log(dataS);
          console.log(dataS.data.summary);
          setCombinedAnswerSM(dataS.data.summary);
        } else {
          const response = await fetch(API_BASE_URL + `/api/pdf/single/${projectId}/${category}`, {
            headers: {
              'Content-Type': 'application/json',
          ...
          console.log(data.data[0].summary);
          console.log('answers getting because no summary');
          setAnswersSM(data.data);
          const combined = data.data.map(answer => {
            const formattedSubType = answer.questionSubType.replace(/([A-Z])/g, ' $1').trim();
            return `<h4 style="text-align: center;">${formattedSubType}</h4><br>${answer.summary}`;
          }).join('<br><br>');
          console.log(combined);
          setCombinedAnswerSM(combined);
          setLoading(false);
        }
      } else {
        throw new Error('Failed to fetch summary');
...
//DetailedMarketingStrategies
useEffect(() => {
  console.log("fetchAnswers");
  const fetchAnswers = async () => {
    const category = 'DetailedMarketingStrategies';
    try {
      const summaryResponse = await fetch(API_BASE_URL + `/api/pdf/${projectId}/${category}`, {
        headers: {
          'Content-Type': 'application/json',
      ...
      if (summaryResponse.ok) {
        ...
        if (dataS.data) {
          console.log(dataS);
          console.log(dataS.data.summary);
          setCombinedAnswerDMS(dataS.data.summary);
        } else {
          const response = await fetch(API_BASE_URL + `/api/pdf/single/${projectId}/${category}`, {
            headers: {
              'Content-Type': 'application/json',
          ...
          console.log(data.data[0].summary);
          console.log('answers getting because no summary');
          setAnswersDMS(data.data);
          const combined = data.data.map(answer => {
            const formattedSubType = answer.questionSubType.replace(/([A-Z])/g, ' $1').trim();
            return `<h4 style="text-align: center;">${formattedSubType}</h4><br>${answer.summary}`;
          }).join('<br><br>');
          console.log(combined);
          setCombinedAnswerDMS(combined);
          setLoading(false);
        }
      } else {
        throw new Error('Failed to fetch summary');
    ...
    <div className='container2'>
       <SideMenu2P />
        ...
        <div>
            <div className='text-center'>
                        <p className='textHp'>Business Analysis Pack</p>
      ...
      <div
          ...
          {/* ValuePropositionPack */}
            <div>
            <div className='text-center'>
                        <p className='textHp'>Value Proposition Pack</p>
